...
- **GitHub Actions** 每日 sync/backfill 写库；**前端由运维自行部署**（Nuxt SSR）
...
.github/    sync.yml（每日同步写库）、backfill.yml（历史回溯）
...
| `XFYUN_MODEL` | 模型 modelId（如 `spark-x`） |
| `NOTIFY_WEBHOOK` | 失败告警 Webhook |

前端部署不经过 Actions；见 [docs/DEPLOY.md](docs/DEPLOY.md)。
