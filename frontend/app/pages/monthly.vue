...
const { data, error } = await useFetch<LeaderboardPayload>('/api/leaderboards/monthly')
const payload = data.value ?? { type: 'monthly', generated_at: null, items: [] }
const { query, language, sortBy, languages, sorted } = useLeaderboard(payload.items, 'monthly')
useHead({
  title: '月增速榜 - GitHub Star 趋势榜',
  meta: [{ name: 'description', content: '近一月 Star 增速最快的 Top 100 开源项目' }],
})
</script>
...
<template>
  <div class="board-page">
    ...
    <div class="mb-2 flex items-baseline justify-between">
      <h1 class="text-lg font-semibold text-foreground">月增速榜</h1>
    ...
    <template v-else>
      ...
      <div v-if="sorted.length" class="board-grid grid gap-4 md:grid-cols-2">
        <div
          ...
          :style="{ '--stagger': index }"
        >
          <RepoCard :item="item" board-type="monthly" />
        </div>
      </div>
      <Alert v-else>
