...
const { data, error } = await useFetch<LeaderboardPayload>('/api/leaderboards/weekly')
const payload = data.value ?? { type: 'weekly', generated_at: null, items: [] }
const { query, language, sortBy, languages, sorted } = useLeaderboard(payload.items, 'weekly')
useHead({
  title: '周增速榜 - GitHub Star 趋势榜',
  meta: [{ name: 'description', content: '近一周 Star 增速最快的 Top 100 开源项目' }],
})
</script>
...
<template>
  <div class="board-page">
    ...
    <div class="mb-2 flex items-baseline justify-between">
      <h1 class="text-lg font-semibold text-foreground">周增速榜</h1>
    ...
    <template v-else>
      ...
      <div v-if="sorted.length" class="board-grid grid gap-4 md:grid-cols-2">
        <div
          ...
          :style="{ '--stagger': index }"
        >
          <RepoCard :item="item" board-type="weekly" />
        </div>
      </div>
      <Alert v-else>
