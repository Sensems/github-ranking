...
const payload = data.value ?? { type: 'total', generated_at: null, items: [] }
...
<template>
  <div class="board-page">
    ...
    <div class="mb-2 flex items-baseline justify-between">
      <h1 class="text-lg font-semibold text-foreground">总 Star 榜</h1>
    ...
    <template v-else>
      ...
      <div v-if="sorted.length" class="board-grid grid gap-4 md:grid-cols-2">
        <div
          ...
          :style="{ '--stagger': index }"
        >
          <RepoCard :item="item" board-type="total" />
        </div>
      </div>
      <Alert v-else>
